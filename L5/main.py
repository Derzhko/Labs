...
for i in range(2, my_column + 1):
    if i < 2 + count_of_product:
        collect.append(float(str(my_sheet_obj.cell(row=row, column=i).value).replace(',', '.')))
    elif 2 + count_of_product <= i < 2 + 2 * count_of_product:
        available.append(float(str(my_sheet_obj.cell(row=row, column=i).value).replace(',', '.')))
    else:
        profit.append(float(str(my_sheet_obj.cell(row=row, column=i).value).replace(',', '.')))
